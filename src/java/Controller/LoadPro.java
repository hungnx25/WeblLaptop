...
public class LoadPro extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        ArrayList<Products> listPro = proDao.getProductList();
        ArrayList<Products> listProDesc = proDao.getProductDesc();
        ArrayList<Customers> listCus = cusDao.getCustomerList();

        HttpSession ses = request.getSession();
        ses.setAttribute("listPro", listPro);
        ses.setAttribute("listProDesc", listProDesc);
        ses.setAttribute("listCus", listCus);

        
        PageInfor page = (PageInfor) ses.getAttribute("page");
        page = new PageInfor(1, 6, listPro.size());
        page.calc();
